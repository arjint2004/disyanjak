...
class Message_model extends Model { 
	...
	function get_messages_hari_ini()
	{
		$y=$this->db->query("SELECT COUNT(*) FROM sentitems WHERE date(SendingDateTime)='".date('Y-m-d')."' ")->result_array();
		//echo $this->db->last_query();
		return $y[0]['COUNT(*)'];
	}
	function get_messages_berhasil_hari_ini()
	{
		$y=$this->db->query("SELECT COUNT(*) FROM sentitems WHERE Status='SendingOKNoReport' AND date(SendingDateTime)='".date('Y-m-d')."'  ")->result_array();
		//echo $this->db->last_query();
		return $y[0]['COUNT(*)'];	
	}
	function get_messages_error_hari_ini()
	{
		$y=$this->db->query("SELECT COUNT(*) FROM sentitems WHERE Status!='SendingOKNoReport' AND date(SendingDateTime)='".date('Y-m-d')."' ")->result_array();
		//echo $this->db->last_query();
		return $y[0]['COUNT(*)'];		
	}
	function get_messages_error_semua()
	{
		$y=$this->db->query("SELECT COUNT(*) FROM sentitems WHERE Status!='SendingOKNoReport'")->result_array();
		//echo $this->db->last_query();
		return $y[0]['COUNT(*)'];			
	}   
    // --------------------------------------------------------------------
